...
import PostGuideView from './post-views/PostGuideView'
...
import PostShareGroup from './post-views/PostShareGroup'
import PostScarperView from './post-views/PostScarperView'
...
export default class PostDetailComponent extends Component {
  ...
  componentDidMount() {
    // console.log('đây là trang bốt đì teo');
  ...
  doGetListCommentPost = (page) => {
    ...
    this.props.getListCommentPost(comment_id, page)
      .then(comment_data => {
        // new comments
        // console.log(page, 'lisst comment', comment_data);
  ...
  doLikePost = (comment_id) => {
    ...
    this.props.likePost(comment_id)
      ...
      .catch(error => {
        // console.log('errorororororr', error);
      });
  }


  doLikePostNotification = (comment_id, notification_id) => {

    // request
    this.props.likePostNotification(comment_id, notification_id)
      .then(data => {
        // console.log("response sau khi thoong baos", data);
        const newLikePost = { ...this.state.postDetail }
        newLikePost.like_comment = data.data.status
        newLikePost.total_likes = parseInt(newLikePost.total_likes) || 0
        if (data.data.status === 'not active') {
          newLikePost.total_likes = newLikePost.total_likes - 1
        } else {
          newLikePost.total_likes = newLikePost.total_likes + 1
        }
        // console.log("12345678", newLikePost);
  ...
  doCreateComment = (data) => {
    // console.log('data bình luận là gì', data, this.props.data);
    const { user_id } = this.props.user.me || ""
    let comments = this.state.comments
    let notification_id = this.props.data.notification_id

    // request
    if (this.props.data && this.props.data.isNotifycation) {

      this.props.createCommentNotification(data, notification_id)
        .then(comment => {
          const postDetail = this.state.postDetail;
          postDetail.total_comments++;
          // console.log('chấm ren com ment', comment);
          // remove fake comment
          _.remove(comments, function (n) {
        ...
        .catch(error => {
          // console.log(comments, 'ê rốt à', error);
          // remove fake comment
          _.remove(comments, function (n) {
            return (n.header.comment_id === FAKE_COMMENT_ID)
          });
    ...
    } else if (this.props.data.isTour == true) {
      this.props.createCommentSearchAll(data)
        .then(comment => {
          const postDetail = this.state.postDetail;
          postDetail.total_comments++;
          // console.log('chấm ren com ment', comment);
    ...
    } else if (this.props.data.isPost == true) {
      this.props.createCommentSearchAllPost(data)
        .then(comment => {
          const postDetail = this.state.postDetail;
          postDetail.total_comments++;
          // console.log('chấm ren com ment', comment);
    ...
    } else if (this.props.data.createCommentProfile) {
      this.props.data.createCommentProfile(data)
        ...
        .then(comment => {
          ...
          const cmt = { header: { ...comment.comment, user_id }, data: replies }
          comments.push(cmt)
          ...
          this.props.data.setNewDataNewfeeds(comment.newfeedsOther)
        })
        .catch(error => {
          // console.log(comments, 'ê rốt à', error);
          // remove fake comment
    ...
    } else if (this.props.data.isTotalTour == true) {
      this.props.createCommentSearchTotalTour(data)
        .then(comment => {
          const postDetail = this.state.postDetail;
          postDetail.total_comments++;
          // console.log('chấm ren com ment', comment);
    ...
    } else if (this.props.data.isTotalPost == true) {
      this.props.createCommentSearchTotalPost(data)
        .then(comment => {
          // console.log('chấm ren com ment', comment);
          // remove fake comment
          const postDetail = this.state.postDetail;
  ...
  onPressLikeButton = () => {
    ...
    if (this.props.data && this.props.data.isNotifycation) {
      ...
      this.doLikePostNotification(comment_id, notification_id)
    }
    else if (this.props.data.isPost == true) {
    ...
    } else if (this.props.data.likeButtonUserProfile == true) {
      this.props.likenewfeedOther(comment_id)
        .then(data => {
          ...
          SocketManager.sendComment()
          this.props.data.setNewDataNewfeeds(data)
        })
        .catch(error => {
          // console.log('errorororororr', error);
        });
  ...
  onPressSendMessage = () => {
    // console.log('đăng com ment', this.props);
  ...
  deleteComment = (value) => {
    ...
    let data = {
      type: typePostDetail.newfeeds,
      comment_id: this.state.postDetail.comment_id
    }
    this.props.deleteComment(value.comment_id, data)
      .then(response => {
    ...
    this.props.applyPost(comment_id)
      .then(data => {
        if (data.currentComments) {
          let newPost = data.currentComments.find(item => item.comment_id === comment_id)
          if (!newPost) {
            this.props.getAgentHomePage({ type: 'tuor', page: this.page })
            newPost = this.props.posts.postAgentHome.find(item => item.comment_id === comment_id);
          }
          this.setState({
            postDetail: newPost
          })
  ...
  renderHeader() {
    ...
    const comment = this.state.postDetail
    // console.log('==================renderHeader const comment = this.state.postDetail==================');
    // console.log(comment);
    // console.log(this.props);
    const photos = _.get(comment, 'photo.photos')
    const detailCommentShare = _.get(comment, 'detailCommentShare')
    const detailGroupShare = _.get(comment, 'detailGroupShare')
    let scraper = comment.scraper
    return (
      <View style={{ flexDirection: 'column' }}>
        ...
        />
        {
          scraper == 'Not scraper' || !scraper || scraper.length == 0
            ?
            null
            :
            <PostScarperView
  ...
  renderSectionHeader = (info) => {
    // console.log(VIEW_PREV_COMMENT, 'in fo bình luận', info);
    ...
    return (
      <View>
        ...
        />
        {
          fakeComment ? <Text style={{ marginLeft: 65, height: 40 }}>{i18next.t('PostingComment')}</Text> :
  ...
  renderShowPrevCommentView() {
    ...
    // console.log('00000000000000000000000000000000', this.state);
