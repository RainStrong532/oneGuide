...
import PostTipView from '../component/post-views/PostTipView'
...
export default class PostDetailTipComponent extends Component {
    ...
    detailHotPost = () => {
        ...
        let id_post = ""
        if (data && data.comment && data.comment.comment_id) id_post = data.comment.comment_id
        // const comment_id = _.get(this.props, 'comment.comment_id')
        let dataDetail = {
            comment_id: id_post
        }
        this.props.getHotPostRelated(null, dataDetail)
            .then(response => {
                this.setState({
                    dataHotDetail: response.data.data.tips
                })
            })
            .catch(error => {
    ...
    doGetListCommentPost = (page) => {
        ...
        this.props.getListCommentPost(comment_id, page)
            .then(comment_data => {
                // new comments
                // console.log('lisst comment', comment_data);
    ...
    doLikePost = (comment_id) => {
        ...
        // request
        this.props.likePostTip(comment_id)
            .then(data => {
    ...
    doCreateComment = (data) => {
        ...
        let comments = this.state.comments
        const isConcern = this.props.options ? this.props.options.isConcern : undefined
        const currentPost = this.props.data

        // request
        if (this.props.options && isConcern) {

            this.props.createCommentConcernTip(data, currentPost)
                .then(comment => {
                    // console.log('chấm ren com ment', comment);
                    // remove fake comment
                    let postDetail = this.state.postDetail
                    // console.log('state post detail tip tip tip tp', postDetail);
            ...
            this.props.createCommentPostTip(data)
                .then(comment => {
                    // console.log('chấm ren com ment', comment);
                    // remove fake comment
                    let postDetail = this.state.postDetail
    ...
    render() {
        ...
        return (
            <View style={[{ marginTop: statusBarHeight }, styles.container]}>
                <View style={{
                    flexDirection: 'row'
                    //  borderBottomColor: '#989BA0', borderBottomWidth: 0.5 
                }}>
    ...
    renderHeader() {
        ...
        return (
            <View style={{ flexDirection: 'column' }}>
                ...
                />
                <View style={{ flex: 1, paddingVertical: 10, borderTopWidth: 5, borderBottomWidth: 5, borderBottomColor: '#D5DBDB', borderTopColor: '#D5DBDB' }}>
                    <View style={{ flex: 1, marginHorizontal: 10, marginBottom: 10 }} >
                        <Text style={{ fontSize: 17, color: 'black', fontWeight: '500' }}>{i18next.t('RelatedPosts')}</Text>
                    </View>
                    <View style={{ flex: 1, }}>
                        {dataHotTing}
                    </View>
    ...
    renderSectionHeader = (info) => {
        // console.log('in fo bình luận', info);
...
class HotPostRelatedComponentWill extends React.Component {
    onPressDetaiPost = (data) => {
        const componentId = this.props.componentId
        gotoPostDetailTip(componentId, { comment: data }, { isConcern: true })
    }
    render() {
        const { dataHot } = this.props || ""
        return (
            <View style={styles.containerTip}>
                <TouchableOpacity style={{ flex: 1, flexDirection: 'row' }} onPress={() => this.onPressDetaiPost(dataHot)}>
