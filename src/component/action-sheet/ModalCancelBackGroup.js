...
class ModalCancelBackGroup extends Component {
    ...
    handleSaveTemp = () => {
        this.dismiss().then(() => {
            this.props.callback('Save_temp')
        })
    }

    handleCancelSave = () => {
        this.dismiss().then(() => {
            this.props.callback('Cancel_save')
        })
    }
                    ...
                    <Animated.View style={{ marginBottom: marginBottom }}>
                        <View style={{
                            ...
                            <TouchableOpacity
                                ...
                                onPress={this.handleSaveTemp}
                            >
                                <Image
                                    style={styles.image_left}
                                    source={Images.share_post}
                                    resizeMode='center' />
                                <Text style={[styles.text]}>{i18next.t('SaveTheDraft')}</Text>
                            </TouchableOpacity>
                            {
                                <TouchableOpacity
                                    style={[styles.button_left]}
                                    onPress={this.handleCancelSave}
                                >
                                    <Image
                                        style={styles.image_left}
                                        source={Images.write_post}
                                        resizeMode='center' />
                                    <Text style={[styles.text]}>{i18next.t('Unsaved')}</Text>
                                </TouchableOpacity>
                            }
...
const ModalCancelBackGroupContainer = connect(
    mapStateToProps,
    {
        deleteComment
    }
)(ModalCancelBackGroup);

export default ModalCancelBackGroupContainer;
