...
export default class PostDetailComponent extends Component {
  ...
  doCreateComment = (data) => {
    // console.log('data bình luận là gì', data);
    const { user_id } = this.props.user.me
    let comments = this.state.comments

    // request
    this.props.createCommentGuideAgent(data)
      .then(comment => {
  ...
  onPressLikeButton = () => {
    ...
    if (this.props.data.isPost == true) {
    ...
    if (this.props.data.isTour == true) {
    ...
    } if (this.props.data.likeButton == true) {
  ...
  doDeleteComment = (comment_id) => {
    ...
    // request
    this.props.deletePost(comment_id, this.state.postDetail.comment_id)
      .then(data => {
        Loading.hideHud()
        this.onPressBack()
      })
